...
function [x, xarray] = newtons_method(x0, tol, N)
    ...
    f = @(x)  (x^4+3*x^2-2*x+2)^(1/2);
    fp =@(x)  (2*x^3+3*x-1)/((x^4+3*x^2-2*x+2)^(1/2));    
  ...
  fprintf('   n\t      x\t\t   f(x) \t\t   quad. ratio \t\t lin. ratio\n');
    ...
    while ( abs(x-x_old) > tol && its < N)
        if (its >= 2)
            fprintf('   %d\t%1.8e\t%1.8e\t\t%1.8e\t\t%1.8e\n',its,x,fx,qratio,lratio);	
            % we can start looking at ratios after 2 iterations
        else
            fprintf('   %d\t%1.8e\t%1.8e\n',its,x,fx);
        end
        x_old = x;
        ...
        its = its+1;
        if its >= 2
            qratio = (xarray(its+1)-xarray(its))/(xarray(its)-xarray(its-1))^2;   
            % compute the quadratic error ratio
            lratio = (xarray(its+1)-xarray(its))/(xarray(its)-xarray(its-1));	  
            % compute the linear error ratio
        end
    end
    fprintf('   %d\t%1.8e\t%1.8e\t\t%1.8e\t\t%1.8e\n',its,x,fx,qratio,lratio);  
    % print the last line after the loop exits
end
